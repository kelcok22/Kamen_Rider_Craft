    ...
    public void render(PoseStack poseStack, T animatable, BakedGeoModel bakedModel, @Nullable RenderType renderType, MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        if (this.getRenderer() instanceof RiderArmorRenderer renderer2) {
            ...
            if (RIDER != null && Objects.requireNonNull(RIDER.getAttribute(Attributes.WIND)).getBaseValue() > 0) {
                ...
                renderType = RenderType.breezeWind(ResourceLocation.fromNamespaceAndPath(KamenRiderCraftCore.MOD_ID, "textures/render_layer/wind.png"), this.xOffset(f) % 1.0F, 0.0F);
                poseStack.mulPose(Axis.YP.rotationDegrees(-(f * 22 - 45.0F)));
                getRenderer().reRender(getDefaultBakedModel(animatable), poseStack, bufferSource, animatable, renderType, bufferSource.getBuffer(renderType), partialTick, packedLight, OverlayTexture.NO_OVERLAY, getRenderer().getRenderColor(animatable, partialTick, packedLight).argbInt());
