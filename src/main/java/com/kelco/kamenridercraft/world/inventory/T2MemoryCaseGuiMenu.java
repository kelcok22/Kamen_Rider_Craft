...
public class T2MemoryCaseGuiMenu extends AbstractContainerMenu {
    ...
    public T2MemoryCaseGuiMenu(int containerId, Inventory inventory, FriendlyByteBuf friendlyByteBuf) {
        this(containerId, inventory, new ItemStack(WRiderItems.T2_MEMORY_CASE.get()));
    }

    public T2MemoryCaseGuiMenu(int containerId, Inventory inventory, FriendlyByteBuf friendlyByteBuf, ItemStack itemStack) {
        this(containerId, inventory, itemStack);
    }

    public T2MemoryCaseGuiMenu(int containerId, Inventory inventory, ItemStack itemStack) {
        ...
        for (int i1 = 0; i1 < 3; ++i1) {
            for (int k1 = 0; k1 < 9; ++k1) {
                this.addSlot(new Slot(inventory, k1 + i1 * 9 + 9, 8 + k1 * 18, 97 + i1 * 18) {
                    @Override
                    public boolean mayPickup(@NotNull Player player) {
                        return !(this.getItem().getItem() instanceof T2MemoryCaseItem);
                    }
                });
            }
        }
        ...
        for (int j1 = 0; j1 < 9; ++j1) {
            this.addSlot(new Slot(inventory, j1, 8 + j1 * 18, 155) {
                @Override
                public boolean mayPickup(@NotNull Player player) {
                    return !(this.getItem().getItem() instanceof T2MemoryCaseItem);
                }
