...
public class FullBottleHolderGuiMenu extends AbstractContainerMenu {
    private final Container container;

    public FullBottleHolderGuiMenu(int containerId, Inventory inventory, FriendlyByteBuf friendlyByteBuf) {
        this(containerId, inventory, new ItemStack(BuildRiderItems.BUILD_DRIVER.get()));
    }

    public FullBottleHolderGuiMenu(int containerId, Inventory inventory, FriendlyByteBuf friendlyByteBuf, ItemStack itemStack) {
        this(containerId, inventory, itemStack);
    }

    public FullBottleHolderGuiMenu(int containerId, Inventory inventory, ItemStack itemStack) {
        super(ModMenus.FULL_BOTTLE_HOLDER_GUI.get(), containerId);
        this.container = new BasicContainer(itemStack, 3);
        container.startOpen(inventory.player);

        this.addSlot(new SlotByTag(container, 0, 80, 16, "gear/fullbottles"));
        this.addSlot(new SlotByTag(container, 1, 80, 34, "gear/fullbottles"));
        this.addSlot(new SlotByTag(container, 2, 80, 52, "gear/fullbottles"));
