...
public class ShiftCarHolderGuiMenu extends AbstractContainerMenu {
    private final Container container;

    public ShiftCarHolderGuiMenu(int containerId, Inventory inventory, FriendlyByteBuf friendlyByteBuf) {
        this(containerId, inventory, new ItemStack(DriveRiderItems.DRIVE_DRIVER.get()));
    }

    public ShiftCarHolderGuiMenu(int containerId, Inventory inventory, FriendlyByteBuf friendlyByteBuf, ItemStack itemStack) {
        this(containerId, inventory, itemStack);
    }

    public ShiftCarHolderGuiMenu(int containerId, Inventory inventory, ItemStack itemStack) {
        super(ModMenus.SHIFT_CAR_HOLDER_GUI.get(), containerId);
        this.container = new BasicContainer(itemStack, 3);
        container.startOpen(inventory.player);

        this.addSlot(new SlotByTag(container, 0, 80, 16, "gear/drive_items"));
        this.addSlot(new SlotByTag(container, 1, 80, 34, "gear/drive_items"));
        this.addSlot(new SlotByTag(container, 2, 80, 52, "gear/drive_items"));
