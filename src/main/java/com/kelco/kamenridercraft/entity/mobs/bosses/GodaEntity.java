...
public class GodaEntity extends BaseHenchmenEntity {
    ...
    public GodaEntity(EntityType<? extends BaseHenchmenEntity> type, Level level) {
        ...
        this.setItemSlot(EquipmentSlot.FEET, new ItemStack(OOORiderItems.GODA_OOODRIVER.get()));
    }
					...
					if (this.attackTime <= 0) {
						...
						if (this.attackStep > 1) {
							...
							for(int i = 0; i < 1; ++i) {
								...
								WitherSkull smallfireball = new WitherSkull(this.CoreEntity.level(), this.CoreEntity, vec3.normalize());
