...
public class MomotarosEntity extends BaseAllyEntity {
    ...
    public void tame(Player p_21829_) {
	   ...
	   this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(DenORiderItems.MOMOTAROSWORD.get()));
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!this.level().isClientSide || this.isBaby() && this.isFood(itemstack)) {
            if (this.isTame()) {
                if (this.isFood(itemstack) && this.getHealth() < this.getMaxHealth()) {
					...
					if (itemstack.is(DenORiderItems.RIDER_PASS.get())) {
						...
						this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(DenORiderItems.DEN_GASHER_SWORD.get()));
            ...
            } else if (itemstack.is(ModdedItemCore.PUDDING) && !this.isAngry()) {
                itemstack.consume(1, player);
            ...
            boolean flag = this.isOwnedBy(player) || this.isTame() || (itemstack.is(ModdedItemCore.VIENNA_COFFEE)||itemstack.is(ModdedItemCore.PUDDING)) && !this.isTame() && !this.isAngry();
