    ...
    public void remove(Entity.RemovalReason p_149847_) {
        ...
        if (this.isDeadOrDying()) {
            ...
            if (chance * 100.0 <= gamerule && (this.lastHurtByPlayer != null && canSpawnBoss(this.lastHurtByPlayer) || !(this.getLastAttacker() instanceof Player) && chance * 200.0 <= gamerule)) {
                if (this.level().getBiome(this.blockPosition()).is(BiomeTags.IS_SAVANNA))
                ...
                else if (this.level().getBiome(this.blockPosition()).is(BiomeTags.IS_MOUNTAIN))
                    boss = MobsCore.ANKH_LOST.get().create(this.level());
                        ...
                        || this.level().getBiome(this.blockPosition()).is(BiomeTags.IS_DEEP_OCEAN))
                    boss = MobsCore.MEZOOL.get().create(this.level());
                else if (this.level().getBiome(this.blockPosition()).is(BiomeTags.IS_BADLANDS))
                    boss = MobsCore.GAMEL.get().create(this.level());
                else if (this.level().getBiome(this.blockPosition()).is(BiomeTags.IS_NETHER))
                    boss = MobsCore.MUCHIRI.get().create(this.level());
                else if (this.level().getBiome(this.blockPosition()).is(BiomeTags.IS_END))
                    boss = MobsCore.KYORYU_GREEED.get().create(this.level());
                else if (this.level().dimension() == SANDS_OF_TIME)
                    boss = MobsCore.SHOCKER_GREEED.get().create(this.level());
                else {
