...
public class ZeinSummonEntity extends BaseSummonEntity {
    ...
    public void actuallyHurt(DamageSource source, float amount) {
		...
		if (!this.level().isClientSide() && source.getEntity() != this.getOwner() && this.getHealth() < this.getMaxHealth() / 2 && this.getItemBySlot(EquipmentSlot.MAINHAND).getItem() instanceof ZeinCard zein) {
			...
			if (isTame()) {
				if (p_28879_ instanceof Creeper || p_28879_ instanceof Ghast) {
					if (this.getMainHandItem().getItem() instanceof BowItem) {
						return !(this.getMainHandItem().getItem() instanceof SwordItem || this.getMainHandItem().is(ItemTags.create(ResourceLocation.fromNamespaceAndPath(KamenRiderCraftCore.MOD_ID, "arsenal/all_swordguns")))) || !this.getMeleeOnly();
					}
					return false;
				}
