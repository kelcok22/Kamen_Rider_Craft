...
public class GenmEntity extends BaseHenchmenEntity {
    ...
    public void actuallyHurt(DamageSource source, float amount) {
        ...
        if(!this.level().isClientSide() && source.getEntity() instanceof Player playerIn) {
            if (this.getHealth()<30 && playerIn.getInventory().countItem(ExAidRiderItems.BAKUSOU_BIKE_GASHAT.get().asItem()) != 0 && this.getItemBySlot(EquipmentSlot.FEET).getItem()!= ExAidRiderItems.GASHACON_BUGVISOR_GENM.get()){
