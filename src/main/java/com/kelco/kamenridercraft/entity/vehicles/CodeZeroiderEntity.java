...
public class CodeZeroiderEntity extends baseBikeEntity {

	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private CodeZeroEntity boss = null;
	private int summonLevel = 21;

	public CodeZeroiderEntity(EntityType<? extends baseBikeEntity> entityType, Level level) {
		super(entityType, level, MobsCore.CODE_ZEROIDER_SPAWN_EGG.get());
		NAME ="code_zeroider";
		NAME_MODEL ="code_zeroider";
	}
		...
		if (!level().isClientSide()) {
			if (this.boss != null) {
			 ...
			 if (player.isShiftKeyDown() && this.level() instanceof ServerLevel && this.boss == null && this.getControllingPassenger() == null){
				 this.boss = MobsCore.CODE_ZERO.get().create(this.level());
