    ...
    public void summon(ItemStack stack, Level level, Player player) {
        ...
        if (summon != null) {
            ...
            if (this.summonWeapons.containsKey(key.toString())) {
                ...
                if (this.summonWeapons.get(key.toString()).length > 1)
                    summon.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(BuiltInRegistries.ITEM.get(ResourceLocation.parse(weapons[1]))));
            }
            if (belt == GaimRiderItems.SENGOKU_DRIVER_ZENGETSU.get())
                RiderDriverItem.setFormItem(summon.getItemBySlot(EquipmentSlot.FEET), GaimRiderItems.ZANGETSU_KACHIDOKI_LOCKSEED.get(), 1);
