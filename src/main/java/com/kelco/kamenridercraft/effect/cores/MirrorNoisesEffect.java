...
public class MirrorNoisesEffect extends MobEffect {
	...
	public boolean applyEffectTick(LivingEntity pLivingEntity, int pAmplifier) {
        ...
        if (pLivingEntity.getEffect(Effect_core.MIRROR_NOISES).getDuration() > 200) {
            pLivingEntity.level().playLocalSound(pLivingEntity, ModSounds.MIRROR_NOISES.get(), SoundSource.RECORDS, 1, rand2);
        }
        if (pLivingEntity.level() instanceof ServerLevelAccessor slevel && pLivingEntity.getEffect(Effect_core.MIRROR_NOISES).getDuration() < 2) {
            if (boss != null) {
